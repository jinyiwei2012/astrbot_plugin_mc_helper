...
| **下载大小限制** | 单次下载 ≤ 200MB，防止磁盘 DoS |
| **SSRF 防护** | DNS 预解析 + Private IP（10.x/127.x/192.168.x 等）拦截 + DNS Pinning |
| **路径穿越防护** | 文件名控制字符清除 + `/`/`../` 开头或 `/../` 检查 + `resolve()` 二次校验 |
| **JSON 内容扫描** | 解压前扫描 `.json` 内容，含危险命令（powershell、wget 等）则拒绝；超大文件分段读取防 OOM |
