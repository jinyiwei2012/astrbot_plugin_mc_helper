...
class McHelperPlugin(Star):
    ...
    async def on_message(self, event: AstrMessageEvent):
        ...
        for file_comp in file_comps:
            ...
            zip_pattern = r"错误报告-(?:\d{4}-\d{1,2}-\d{1,2}|\d{1,2}-\d{1,2}-\d{4})_\d{1,2}\.\d{2}\.\d{2}\.zip"
    ...
    async def _handle_error_report(self, event: AstrMessageEvent, file_comp: File):
        ...
        try:
            ...
            if "ConcurrentModificationException" in error_logs:
                has_cme_log = (extract_dir / "CMESuckMyDuck.log").is_file()
                if has_cme_log:
                    cme_analysis = self._analyze_cme_log(extract_dir / "CMESuckMyDuck.log")
                    if cme_analysis:
                        yield event.plain_result(cme_analysis)
                        return
                else:
                    cme_guide = self._generate_cme_guide(error_logs)
                    if cme_guide:
                        async for r in self._send_md_image(event, cme_guide):
                            yield r
                        return
    ...
    def _analyze_cme_log(self, log_path: Path) -> str | None:
        try:
        ...
        for line in content.split("\n"):
            ...
            if m:
                thread_traces.setdefault(m.group(1).strip(), 0)
                thread_traces[m.group(1).strip()] += 1
    ...
    def _generate_cme_guide(self, error_text: str) -> str | None:
        stacks = _RE_STACK.findall(error_text)
        meaningful = [s for s in stacks if not s[0].startswith(_SKIP_STACK)]
        if not meaningful:
            return None
